...
2.  **Select Code Style Guides:**
    -   List the available style guides by using the `Shell` tool to execute `ls PLUGIN_TEMPLATES/code_styleguides/`. **CRITICAL: You MUST use `Shell` for this step. Do NOT use the `Glob/Read` tool, as the templates directory resides outside of your allowed workspace and the call will fail.**
...
2.  **Initialize Tracks File:** Create the `.cursor/context/tracks.md` file with the initial header and the first track:
    ...
    This file tracks all major tracks for the project. Each track has its own spec and Cursor plan.
